namespace IAmBacon.Domain.Membership
{
    ...
    using IAmBacon.Domain.Encryption;
    using IAmBacon.Model.Entities;

    /// <summary>
    /// The custom membership provider.
    /// </summary>
    public class BaconMembershipProvider : MembershipProvider
    {
        #region Fields
        ...
        /// </summary>
        /// <returns>
        /// Name of the application using the custom membership provider.
        /// </returns>
        ...
        /// Couldn't get [Inject] attribute to work so using this for now.
        /// </summary>
            ...
            out MembershipCreateStatus status)
        {
            ...
            if (this.RequiresUniqueEmail && this.GetUserNameByEmail(email) != null)
            {
            ...
            if (user == null)
            {
                ...
                encrypt.GetHashAndSaltString(password, out hash, out salt);
            ...
            out MembershipCreateStatus status)
        {
            ...
            if (user == null)
            {
                ...
                // Todo: should probably return a BaconMembership User. Will see if needed.
                return this.GetUser(username, true);
            }
        ...
        {
            User user = UserRepository.Find(x => x.Email == username && x.Active == userIsOnline).FirstOrDefault();
        ...
        public override string GetUserNameByEmail(string email)
        {
            ...
            User user = UserRepository.Find(x => x.Email == email).FirstOrDefault();
        ...
        {
            var user = UserRepository.Find(x => x.Email == username && x.Active).FirstOrDefault();

            if (user != null)
            {
                var encrypt = new EncryptionManager();

                var result = encrypt.VerifyHashString(password, user.Password, user.Salt);
